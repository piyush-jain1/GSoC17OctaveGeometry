...
DEFUN_DLD(polybool, args, , 
           "\
-*- texinfo -*-\n\
@deftypefn {Loadable Function} {} polybool (@var{subj} ,@var{clip}, @var{operation}\n\
...
{ 
  ...
  { 
    ...
    if(size >= 0)
    {
        ...
        octave_idx_type k = 0;
        for (unsigned int i = 0; i < martinezResult.ncontours (); i++) 
        {
